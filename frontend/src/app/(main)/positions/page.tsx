...
import type { PaginatedResponse, PositionBrief } from "@/types";
...
export default function PositionsPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<PositionBrief>>({
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mb-6">
            {(data?.items || []).map(pos => (
              <div key={pos.id} className="p-4 rounded-xl border transition-all hover:-translate-y-0.5 group"
                ...
                <div className="text-xs font-medium mb-2 truncate" style={{ color: "var(--color-primary-light)" }}>
                  {pos.category_name || pos.recruit_type}
                </div>
                <h3 className="font-semibold text-sm mb-2 line-clamp-2 leading-snug">{pos.name}</h3>
                <div className="text-xs mb-1 truncate" style={{ color: "var(--color-text-dim)" }}>
                  {pos.company?.name || "未知公司"}
                </div>
                <div className="flex items-center gap-3 text-xs" style={{ color: "var(--color-text-dim)" }}>
                  {pos.city && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{pos.city}</span>}
                  {pos.salary_text && <span style={{ color: "var(--color-accent-light)" }}>{pos.salary_text}</span>}
                </div>
                {pos.tags && (
